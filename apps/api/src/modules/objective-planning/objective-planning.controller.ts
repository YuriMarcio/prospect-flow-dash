...
import {
  ...
  getSessionOrThrow,
} from "./objective-planning.service";
...
export async function confirmSessionController(
  request: FastifyRequest<{ Params: { id: string }; Body: { columnId: string } }>,
  reply: FastifyReply,
) {
  try {
    const session = await requireOwnSession(request);
    const result = await confirmSession(session, request.body.columnId);
